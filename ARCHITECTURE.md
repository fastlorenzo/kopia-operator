...
type KopiaRepositorySpec struct {
    ...
    // Authentication
    RepositoryPassword                string  // Direct password (not recommended)
    RepositoryPasswordExistingSecret  string  // Reference to Secret containing password
...
1. **Secret Reference** (Recommended): `repositoryPasswordExistingSecret`
2. **Direct Password**: `repositoryPassword` (not recommended for production)

The secret must contain a key named `KOPIA_PASSWORD`.
