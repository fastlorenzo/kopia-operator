...
1. **Repository Encryption Password** (`repositoryPassword`) - Encrypts/decrypts repository data
2. **Server Admin Password** (`serverAdminPassword`) - Controls server API access
...
metadata:
  name: my-repo
spec:
  repositoryPassword: "repository-encryption-password"
  server:
    enabled: true
...
spec:
  ...
  server:
    enabled: true
    serverAdminPasswordExistingSecret: "kopia-server-admin-password"
```

The secrets should contain:

- Repository password secret: key `password`
- Server admin password secret: key `password`

### Auto-Created Secrets

When using direct password values, the operator automatically creates secrets:

- Repository password: `kopia-repo-<repository-name>`
- Server admin password: `kopia-server-admin-<repository-name>`

## Default Behavior

If `serverAdminPassword` is not specified:

- The operator falls back to using `repositoryPassword` for both purposes
...
1. **Server Admin Password**:
   ...
   - Update the secret or `.spec.server.serverAdminPassword`
